...
__export(main_exports, {
  NOTION_TABLE_VIEW: () => NOTION_TABLE_VIEW,
  NotionTableView: () => NotionTableView,
...
var NotionBasesPlugin = class extends import_obsidian.Plugin {
  async onload() {
    if (typeof this.registerBasesView !== "function") {
      new import_obsidian.Notice("Notion Bases: requires Obsidian 1.10.0+ (registerBasesView API missing).", 8e3);
      return;
    }
    ...
    if (!ok) {
      new import_obsidian.Notice("Notion Bases: view registration failed. Is the Bases core plugin enabled?", 8e3);
    }
  }
};
var NotionTableView = class extends import_obsidian.BasesView {
  ...
  renderCell(td, entry, prop) {
    ...
    if (value instanceof import_obsidian.BooleanValue) {
    ...
    if (editable) {
      td.addClass("ntn-editable");
      const kind = value instanceof import_obsidian.NumberValue ? "number" : "text";
  ...
  editCell(td, entry, propName, current, kind) {
    if (td.querySelector("input.ntn-input")) return;
    td.empty();
  ...
  async writeProperty(entry, propName, value) {
    try {
      ...
      new import_obsidian.Notice(`Couldn't update "${propName}".`);
      this.onDataUpdated();
    }
  }
  valueToStrings(value) {
    if (value === null) return [];
    if (value instanceof import_obsidian.ListValue) {
      const out = [];
  ...
  openSelectEditor(td, entry, prop, propName) {
    ...
    let selected = this.valueToStrings(entry.getValue(prop)).map(
      (s) => s.replace(/^#/, "")
    );
    ...
    const write = () => {
      const out = isList ? selected.length ? selected : null : selected[0] ?? null;
      void this.writeProperty(entry, propName, out);
    };
    const renderPills = () => {
      pillsWrap.empty();
    ...
    const pick = (v) => {
      if (isList) {
        ...
        selected = has ? selected.filter((s) => s.toLowerCase() !== v.toLowerCase()) : [...selected, v];
    ...
    const renderOptions = () => {
      ...
      const visible = [...known.values()].filter((o) => !ql || o.toLowerCase().includes(ql)).sort((a, b) => a.localeCompare(b, void 0, { sensitivity: "base" }));
    ...
    const onDocDown = (evt) => {
      if (!menu.contains(evt.target)) this.closeSelectMenu();
    };
